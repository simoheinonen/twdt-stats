    ...
    foreach ($matches as $i => $match) {
        ...
        foreach (explode("\n", $match)  as $index => $row) {
            if ($index === 0) {
                ...
                if ($l === 'twdtb') {
                    preg_match('/[[](?P<league>.*)[]]\s+(?P<home>.*)[ ]vs[.][ ](?P<away>.*?(?=\s{2}))\s+(?P<home_score>\d+:\d+) - (?P<away_score>\d+:\d+)/', $row, $matches);
                } else {
...
function getShip(string $row, string $league) {
    ...
    if (strstr($row, '(S)') !== false) {
        return 'Shark';
    }

    if (strstr($row, '=T=') !== false) {
        return 'Terrier';
    }


    if (strstr($row, '<J>') !== false) {
        return 'Javelin';
    }

    if (strstr($row, '<W>') !== false) {
        return 'Warbird';
    }


    if (strstr($row, '<L>') !== false) {
        return 'Lancaster';
    }
